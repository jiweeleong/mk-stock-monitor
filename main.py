...
def generate_report(df):
    ...
    <body>
        ...
        <p>趋势基于 MA5>MA20 上升，MA5<MA20 下跌，否则盘整</p>
        <p>按趋势强度从强到弱排序</p>
        <table>
            <tr>
                <th>代码</th>
    ...
    for _, row in df.iterrows():
        ...
        if isinstance(strength_val, (int, float)):
            strength_class = "strength-positive" if strength_val > 0 else "strength-negative"
...
def main():
    ...
    logger.info(f"定时模式：每天 {SCHEDULE_HOUR}:{SCHEDULE_MINUTE} 执行")
    while True:
        now = datetime.now()
